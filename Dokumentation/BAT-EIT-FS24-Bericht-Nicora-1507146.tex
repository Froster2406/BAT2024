...
\end{table}
	...
	Um aus dem 1-Bit PDM-Datenstream ein verarbeitbares PCM-Signal zu erhalten, wird die integrierte CIC-Filterstruktur verwendet. Gemäss \cite{noauthor_efr32xg22_2022}, \ref{Dynamikbereich} sowie \cite{noauthor_httpsinvensensetdkcomwp-contentuploads202007ds-000157-ics-41351-v14pdf_nodate} muss diese folgendermassen konfiguriert sein:
	\begin{itemize}[topsep=10pt,partopsep=0pt,labelwidth=5cm,align=left,itemindent=5cm]
	...
	\item[$\bullet$ Signal-Abtastrate:]  48 kHz
	\end{itemize}
	\begin{equation}\label{eq:Clock-Dezimierung}
		\color{red}TODO\color{black}
		\underbrace{27 \mu A/MHz \cdot 76.8MHz}_{ \mu C} + \underbrace{10 \mu A/MHz \cdot 76.8MHz}_{Peripherie} = 2842 \mu Ah
	\end{equation}
...
\end{table}
	\paragraph{Übersicht gemessen}
		...
		\end{table}
	Wie den Tabellen \ref{tab:leistung-gemessen} und \ref{tab:leistung-berechnet} zu entnehmen ist, kann die Einsatzdauer von 12 Stunden mit einem 50mAh-Akku abgedeckt werden.
	\subsection{Mikrofon-Messungen}
	Das MEMS-Mikrofon, sowie das M2211 von NTI, wurden beide jeweils vor jeder Messung mittels dem CAL200 auf 94dBSPL kalibriert. Genauere Angaben sind dem Anhang \ref{Messmittel} zu entnehmen.
	...
	\paragraph{BLE}\mbox{}\\
	Der BLE-Stack weist eine hohe Komplexität auf. Dadurch wird dieser grundsätzlich nicht selbst implementiert, sondern vom Hersteller des Mikrocontrollers zur Verfügung gestellt. Dies birgt jedoch die Gefahr, dass gewisse Peripherien einen Doppelnutzen erfahren wie dies beispielsweise auch bei der RTC der Fall ist. Genauso war es auch der \color{red}TODO\color{black}
	\paragraph{Mikrofon}\mbox{}\\
	\color{red}TODO\color{black}
	...
	\begin{itemize}
		...
		\item Low-Power \\
		Bei der Entwicklung wurde ein besonders Augenmerk auf die Energieeffizienz gelegt. Nichts desto trotz, kann das System noch energieeffizienter programmiert werden. Dies würde gleichzeitig nochmals einen kleinere Akku oder eine längere Akkulaufzeit zur Folge haben. Insbesondere ein Mikrocontroller mit integrierter BiQuad-Hardware würde die hohe Berechnungszeit \ref{tab:ausfuehrungsgeschwindigkeit} reduzieren
	\end{itemize}
